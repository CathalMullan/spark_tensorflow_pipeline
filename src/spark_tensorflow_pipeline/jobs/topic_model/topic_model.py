...
def make_encoder(activation: str, num_topics: int, layer_sizes: List[int]) -> Callable[[tf.Tensor], tfd.Dirichlet]:
    ...
    :return: A callable mapping a bag-of-words Tensor to tfd.Distribution instances over topics.
    """
...
def make_decoder(num_topics: int, num_words: int) -> Tuple[Callable[[tf.Tensor], tfd.OneHotCategorical], tf.Variable]:
    ...
    :return: A callable mapping a Tensor of encodings to tfd.Distribution instances over words.
    """
...
def parse_dataset(matrix: Union[str, csr_matrix], num_words: int, shuffle_and_repeat: bool) -> tf.data.Dataset:
    """
    ...
    # Dataset stored as a sparse matrix outside of the graph.
...
def build_input_fns(
    ...
    """
    Build iterators for 'train' and 'evaluation' data. Objects represented as a bag-of-words vector.
    ...
    def train_input_fn() -> tf.data.Dataset:
        ...
        train_dataset = parse_dataset(
            matrix=f"{data_source}train_body.npz", num_words=num_words, shuffle_and_repeat=True
        )
    ...
    def eval_input_fn() -> tf.data.Dataset:
        ...
        eval_dataset = parse_dataset(
            matrix=f"{data_source}test_body.npz", num_words=num_words, shuffle_and_repeat=False
        )
...
def main() -> None:
    ...
    train_input_fn, eval_input_fn, vocabulary = build_input_fns(batch_size=parameters.batch_size)
    print("Created input functions.")
    ...
    iterations = parameters.max_steps // parameters.viz_steps
    for iteration in range(iterations):
        ...
        print(f"\nworker: {hvd.rank}: iteration: {iteration}/{iterations}: {time.time() - start_time}")
